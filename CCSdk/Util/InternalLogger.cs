...
namespace CCSdk
{
  ...
  public static class InternalLogger
  {
    ...
    public static string RenderLog(params object[] objs)
    {
      StringBuilder result = new StringBuilder();

      //result.Append(DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss.fffffffUzzz", CultureInfo.InvariantCulture));
      result.Append(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fffffff", CultureInfo.InvariantCulture));
      result.Append("|INTL|");
        ...
        foreach (var item in objs)
        {
          ...
          if (item.GetType().GetProperties().Any())
          {
            ...
            foreach (var p in item.GetType().GetProperties())
            {
              ...
              {
                result.Append(p == null ? string.Empty : p.Name + "=" + p.GetValue(item) + "&");
              }
              else
              {
                result.Append(p == null ? string.Empty : p.Name + "=" + p.GetValue(item) + "||");
